...
public class FourierCPU
{
	...
	public int PeformFFT(int startIdx, Vector2[,] data)
	{
		int x; int y; int i;
		...
		for (i = 0; i < m_passes; i++, j++) 
		{
			...
			for(x = 0; x < m_size; x++)
			{
				for(y = 0; y < m_size; y++)
				{
					...
					data[idx, x+y*m_size] = FFT(w, data[idx1, X+y*m_size], data[idx1, Y+y*m_size]);
				}
			}
		}
		...
		for (i = 0; i < m_passes; i++, j++) 
		{
			...
			for(x = 0; x < m_size; x++)
			{
				for(y = 0; y < m_size; y++)
				{
					...
					data[idx, x+y*m_size] = FFT(w, data[idx1, x+X*m_size], data[idx1, x+Y*m_size]);
				}
			}
		}
	...
	public int PeformFFT(int startIdx, Vector4[,] data)
	{
		int x; int y; int i;
		int idx = 0; int idx1; int bftIdx;
		int X; int Y;
